...
#PBS -l wd
#PBS -l jobfs=1GB
#PBS -l storage=gdata/w97+gdata/hh5+gdata/oi10+gdata/ua8+gdata/fs38+gdata/rr3
...
DIR="/g/data/w97/amu561/CABLE_AWRA_comparison/CORDEX_data"
...
search_criteria="$dataset --domain AUS-44i --experiment historical \
                 ...
                 --experiment rcp26 --variable mrros --time_frequency mon"
...
IN_DIR=$DIR"/Raw_CORDEX_data"
mkdir -p $IN_DIR

#Directory for processed folder
OUT_DIR=$DIR"/Processed_CORDEX_data/"
mkdir -p $OUT_DIR
...
in_file=$TEMP_DIR/"${dataset}_clef_search_results_mrros.csv"
...
do
    ...
    vars="${IN_DIR}/${E}/mon/mrro" #`find ${IN_DIR}/${E} -maxdepth 2 -mindepth 2 ! -path . -type d`
    ...
    do
        ...
        #Find models
        gcm_models=`find $V -maxdepth 1 -mindepth 1 ! -path . -type d | sed 's!.*/!!'`
        ...
        do
            ...
            echo "Processing GCM model : ${M}"
            
            rcm_models=`find $V/$M -maxdepth 1 -mindepth 1 ! -path . -type d | sed 's!.*/!!'`
            ...
            do
              ...
              do
                ...
                files=`find $V/$M/$R/$ens/*.nc`

                #Determine number of files
                no_files=`find $V/$M/$R/$ens/*.nc | wc -l`
                ...
                #Create output path
                processed_path="${OUT_DIR}/${E}/${var_short}/${M}/${R}/${ens}/"
                ...
                if [ -e ${ext_files[0]} ]; then
                  echo "${M} - ${R} - ${var_short} already processed, skipping"
                  continue
                fi
                ...
                then
                    ...
                    out_merged="${processed_path}/Merged_${var_short}_${M}_${R}_${E}_${ens}.nc"
                ...
                processed_file="${processed_path}/${var_short}_${M}_${R}_${E}_${ens}"
