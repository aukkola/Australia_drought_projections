...
sorted_results <- as.data.frame(matrix(data=NA, nrow=nrow(results), ncol=9))
colnames(sorted_results)  <- c("gcm_model", "rcm_model", "experiment", "ensemble", "variable", 
                               "domain", "version", "time_resolution", "path")
...
#Get GCMs
sorted_results$gcm_model      <- sapply(all_res, function(x) x[10])

#Get RCMs
sorted_results$rcm_model      <- sapply(all_res, function(x) x[13])


#Get experiments
sorted_results$experiment <- sapply(all_res, function(x) x[11])

#Get ensemble member
sorted_results$ensemble   <- sapply(all_res, function(x) x[12])

#Get variable
sorted_results$variable   <- sapply(all_res, function(x) x[16])

#Get grid type
sorted_results$domain       <- sapply(all_res, function(x) x[8])

#Get version
sorted_results$version    <- sapply(all_res, function(x) x[17])

#Get time resolution
sorted_results$time_resolution <- sapply(all_res, function(x) x[15])
...
#Variables to match
match <- final_results[,c("gcm_model", "rcm_model", "domain", "experiment", "ensemble", "variable")]
...
for (k in 1:nrow(final_results)) {
  ...
  if (combine) {
    ...
    target_dir <- paste(outdir, dir_name, entry$time_resolution,
                        entry$variable, entry$gcm_model, entry$rcm_model,
                        entry$ensemble, sep="/")
  ...
  #Model check
  if (!grepl(entry$gcm_model, entry$path)) {
