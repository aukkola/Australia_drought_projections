...
root_path = "/g/data/w97/amu561/CMIP6_runoff_drought/"
...
var_name=["mrro"]#, "mrro", "mrros"]    #Variable name in netcdf file
        ...
        for m in range(len(models)):
            ...
            for e in range(len(ensembles)):
                ...
                out_file = (out_path + '/' + models[m] + "_" + ensembles[e] + 
                            '_drought_metrics_perc_' + str(perc_onset) + "_" + 
                            str(perc_termination))
                    ...
                    for j in range(len(lon)):
                         ...
                         len(np.unique(data[:,i,j][~np.isnan(data[:,i,j])])) > 1) :  
                             ...
                             metric = drought_metrics_two_threshold(mod_vec=data[:,i,j], lib_path=lib_path,
                                                      ...
                                                      add_metrics=(['rel_intensity', 
                                                      'threshold_onset', 'threshold_termination', 'timing']),
                                                      subset=subset)
