...
#Experiments
experiments <- list.files(paste0(path, "/Global_mean_temp/CORDEX/"), pattern="rcp")
...
#Output directory
outdir <- paste0(path, "/Global_warming_levels/CORDEX/")
...
for (exp in 1:length(experiments)) {
  ...
  #List GCMS
  gcm_models <- list.files(paste0(path, "/Global_mean_temp/CORDEX/", experiments[exp]))
  
  
  #Loop through GCMs
  for (m in 1:length(gcm_models)) {
    
    #List RCMs
    rcm_models <- list.files(paste0(path, "/Global_mean_temp/CORDEX/", experiments[exp], 
                                    "/", gcm_models[m]))
    
    for (r in 1:length(rcm_models)){
      
      
      ensembles <-  list.files(paste0(path, "/Global_mean_temp/CORDEX/", experiments[exp], 
                                      "/", gcm_models[m], "/", rcm_models[r]))
      ...
      for (e in 1:length(ensembles)) {
        ...
        #Get historical data 
        hist_file <- list.files(paste0(path, "/Global_mean_temp/CORDEX/historical/", 
                                       gcm_models[m], "/", rcm_models[r], "/", ensembles[e]), full.names=TRUE)
        
        if (length(hist_file) == 0) {
          print(paste0("Skipping: ", gcm_models[m], ", ", rcm_models[r], ", ", 
                       ensembles[e], ", no historical file available"))
          next
        }
        ...
        #Get future data
        fut_file <- list.files(paste0(path, "/Global_mean_temp/CORDEX/",
                                      experiments[exp], "/", gcm_models[m], "/", 
        ...
        #Get data years
        #Note this function doesn't work correctly for some files
        #that have time units "minutes since XXX". It has an offset for some
        #reason but seems to work correctly to identify the start year
        start_yr <- format(as.Date(read_nc_time(hist_file)), format="%Y")[1]
        
        
        if (start_yr > 1971) {
          print(paste0("Skipping: ", gcm_models[m], ",", rcm_models[r], ",", 
                       ensembles[e], ", wrong historical start year (", start_yr, ")"))
          next
        }
        ...
        for (b in 1:length(baseline_start)) {
          ...
          outdir_mod <- paste0(outdir, "/baseline_", baseline_start[b], "_",  baseline_end[b],
                               "/", experiments[exp], "/", gcm_models[m], "/", rcm_models[r],
                               "/", ensembles[e])
          ...
          outfile <- paste0(outdir_mod, "/Monthly_indices_global_warming_levels_", min(envelopes), 
                            "-", max(envelopes), "deg_", experiments[exp], "_", gcm_models[m],
                            "_", rcm_models[r], "_", ensembles[e], ".rds")
