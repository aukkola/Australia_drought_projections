          ...
          for (m in 1:length(metrics)) {
            ...
            if (!file.exists(hist_outfile)) {
              ...
              if (metrics[[m]] == "timing") {
                ...
                hist_mean <- sum(hist_data[[start_ind : end_ind]], na.rm=TRUE) /
                  length(start_ind : end_ind) * 100
                
              } else {
                #Take mean of the metric
                hist_mean <- mean(hist_data[[start_ind : end_ind]], na.rm=TRUE)
              }
            ...
            for (w in 1:length(warming_levels)) {
              ...
              if (!file.exists(fut_outfile)) {
                ...
                if (metrics[[m]] == "timing") {
                  ...
                  #Calculate time under drought (in %)
                  fut_mean <- sum(all_data[[inds]], na.rm=TRUE) /
                    length(inds) * 100
                  
                } else {
                  #Take mean of the metric
                  fut_mean <- mean(all_data[[inds]], na.rm=TRUE)
                }
