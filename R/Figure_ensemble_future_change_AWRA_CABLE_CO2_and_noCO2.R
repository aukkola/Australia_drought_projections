...
vars_all <- list(#pr   = list(CMIP6 = "pr", BARPA = "pr", AWRA = "pr"),
                 qtot = list(AWRA = "qtot", CABLE="qtot", CABLE_noCO2="qtot"),
                 sm   = list(AWRA = "sm", CABLE="sm", CABLE_noCO2="sm"))
...
datasets <- c("AWRA", "CABLE", "CABLE_noCO2")
...
for (m in 1:length(metrics)) {
  ...
  for (v in 1:length(vars)) {
    ...
    ### Set up figure ###
    png(paste0(outdir, "/FigureX", "_AWRA_CABLE_CO2_noCO2_ensemble_changes_in_", metrics[m], "_",
  ...
  #  } else {
   ...
   # }
    ...
    for (d in 1:length(datasets)) {
      ...
      if (datasets[d] == "CABLE_noCO2") {
        dataset_pattern <- "CABLE"
      } else{
        dataset_pattern <- datasets[d]
        
      }
      model_path <- paste0(path, "/Mean_drought_metrics/", dataset_pattern, 
      ...
      if (datasets[d] == "CABLE") {
        pattern="/CO2/" 
      } else if (datasets[d] == "CABLE_noCO2") {
        pattern="/noCO2/"
      }
      ...
      if (any(!file.exists(c(hist_out_file, unlist(fut_out_files))))) {
        ...
        for (mod in 1:length(models)) {
          ...
          mod_path <-  paste0(path, "/Mean_drought_metrics/", dataset_pattern, 
                              "/scale_", scale, "/", vars_all[[vars[v]]][[datasets[d]]], 
                              "/", metrics[m])
