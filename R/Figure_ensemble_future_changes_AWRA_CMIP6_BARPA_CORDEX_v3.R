...
lims_diff <- list(duration        = c(-1000, -3, -2, -1, -0.5, 0, 0.5, 1, 2, 3, 1000),
                  ...
                  time_in_drought = c(-100, -10, -8, -6 , -4, -2, 0 , 2, 4, 6, 8, 10, 100))
                    ...
                    # c(-1000, -15, -7.5, -4.5, -3, -1.5, 0,
                    #                   1.5, 3, 4.5, 7.5, 15, 10000))
                    
#c(-100, -15, -11.25, -7.5 -3.25, -1.5, 0, 1.5, 3.25, 7.5, 11.25, 15, 100)) #c(-100, -10, -8, -6 , -4, -2, 0 , 2, 4, 6, 8, 10, 100))
#c(-100, -50, -30, -20, -10, 0, 10, 20, 30, 50, 100)

unit <- c(duration        = "months", 
          rel_intensity   = "% points",
          time_in_drought = "Future change (%.)")
...
for (m in 1:length(metrics)) {
  ...
  for (v in 1:length(vars)) {
    ...
    ### Set up figure ###
    png(paste0(outdir, "/FigureXXX", "_ensemble_changes_in_", metrics[m], "_",
               percentile, "_", scale, "_", vars[v], "_v3.png"),
    ...
    # } else {
    layout(matrix(c(1:2, 3, 4:5, 3, 6:7, 3, 8:9,3), nrow=3, byrow=FALSE), heights=c(1,1,0.3)) #(rep(1,4 ), 0.3) )
    #    }
    ...
    for (d in 1:length(datasets)) {
      ...
      for (env in 1:length(envelopes)) {
        ...
        #time period label
        if (d == 1) mtext(side=2, paste0(env, " degree"), line=1.5)
        
        if(env ==1) mtext(side=3, datasets[d])
      ...
      if(d ==1 ) {
        ...
        legend_lims <- lims_diff[[metrics[m]]][2:len1]
        # if (metrics[m]=="time_in_drought") {
        #   legend_lims <- c(-100, -50, -30, -20, -10, 0, 10, 20, 30, 50, 100)
        # }
         
        #Legend
        add_raster_legend2(cols=cols_diff(len1), limits=legend_lims,
                           main_title=unit[metrics[m]], plot_loc=c(0.2,0.8,0.40, 0.60),
                           title.cex=1.2, spt.cex=1, clip=TRUE, ysp_title_old=FALSE)
